...
from pathology.transformation_pipeline import local_main
...
_CONTAINER_BASE_DIR = local_main._CONTAINER_BASE_DIR
...
def _make_container_dirs(container_dir: local_main._ContainerDirs) -> None:
  for path in (
...
class LocalhostMainTest(absltest.TestCase):
  ...
  def tearDown(self):
    ...
    local_main._CONTAINER_BASE_DIR = _CONTAINER_BASE_DIR
    super().tearDown()

  def test_call_if_no_files_succeeds(self):
    self.assertIsNone(local_main._call_if_no_files())
  ...
  def test_cannot_read_write_metadata_dirs_raises(self):
    ...
    with flagsaver.flagsaver(
      ...
      with self.assertRaises(local_main._UnableToReadWriteFromDirError):
  ...
  def test_ingest_flat_image_file_list(self, unused_mock):
    with flagsaver.flagsaver(
      ...
      container_dirs = local_main._get_container_dirs()
  ...
  def test_local_main_missing_dicom_store_def_raises(self):
    ...
    with flagsaver.flagsaver(
      ...
      _make_container_dirs(local_main._get_container_dirs())
      with self.assertRaises(
          local_main._LocalhostMissingDicomStoreConfigurationError
      ):
        local_main.main(unused_argv=None)
  ...
  def test_build_ingest_file_list_returns_none_if_polling(self):
    self.assertIsNone(local_main._build_ingest_file_list('foo'))

  def test_has_metadata_schema_in_dir_false(self):
    self.assertFalse(
        local_main._has_metadata_schema_in_dir(
  ...
  def test_copy_default_schema_to_metadata_dir(self):
    ...
    local_main._copy_default_schema_to_metadata_dir(temp_dir)
    self.assertEqual(
  ...
  def test_ignore_non_schema_files(self):
    ...
    with open(os.path.join(temp_dir, 'foo_magic.bad'), 'wt') as outfile:
      outfile.write('foo')
    old_path = local_main._EXAMPLE_METADATA_MAPPING_SCHEMA_PATH
    try:
      local_main._EXAMPLE_METADATA_MAPPING_SCHEMA_PATH = temp_dir
      temp_test_dir = self.create_tempdir().full_path
      local_main._copy_default_schema_to_metadata_dir(temp_test_dir)
      self.assertEqual(
          set(os.listdir(temp_test_dir)), _EXAMPLE_METADATA_MAPPING_SCHEMAS
      )
    finally:
      local_main._EXAMPLE_METADATA_MAPPING_SCHEMA_PATH = old_path

  def test_copy_default_schema_to_metadata_dir_consume_os_error(self):
    self.assertIsNone(
        local_main._copy_default_schema_to_metadata_dir('bad_dir')
    )
