...
def _get_arg_parser() -> argparse.Namespace:
  ...
  arg_parser.add_argument(
      ...
      default=os.getenv('DOCKER_CONTAINER_NAME', 'localhost'),
  ...
  arg_parser.add_argument(
      ...
      default=os.getenv(
          'RUNNING_DOCKER_INSTANCE_NAME', 'localhost_transform_pipeline'
      ),
...
def _copy_file_to_image_ingestion_dir(source: str, dest: str) -> None:
  ...
  if source.lower().startswith(gs_prefix):
    ...
    try:
      subprocess.run(
          ['gsutil', '-m', 'cp', '-r', source.rstrip('/'), dest.rstrip('/')],
          check=True,
      )
...
def _test_dicom_store() -> None:
  ...
  if dicom_store is None:
    raise _StartLocalHostError(
        'Required parameter dicom_store is undefined. Should be set to local'
        ' director or URL to GCP DICOM Store.'
    )
  if _is_dicom_store_gcp_store():
    ...
    return
  if not os.path.isdir(dicom_store):
    raise _StartLocalHostError(
