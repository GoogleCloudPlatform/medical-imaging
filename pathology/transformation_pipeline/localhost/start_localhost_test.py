...
from pathology.transformation_pipeline.localhost import start_localhost

_LINUX_ADC_PATH = start_localhost._LINUX_ADC_PATH
_WINDOWS_ADC_PATH = start_localhost._WINDOWS_ADC_PATH
...
def _get_mounts(args: List[str]) -> Mapping[str, str]:
  mount_dict = {}
  for mount in start_localhost._get_argument_settings('--mount', args):
...
class _ArgParserFlagsaver(contextlib.ContextDecorator):
  ...
  def __enter__(self):
    self._pre_existing_flags = start_localhost._parsed_args
    existing_flag_values = copy.deepcopy(vars(start_localhost._parsed_args))
    ...
    start_localhost._parsed_args = argparse.Namespace(**existing_flag_values)

  def __exit__(self, *exc):
    start_localhost._parsed_args = self._pre_existing_flags
...
class StartLocalhostTest(parameterized.TestCase):
  ...
  def setUp(self):
    super().setUp()
    start_localhost._parsed_args = start_localhost._get_arg_parser()
    start_localhost._LINUX_ADC_PATH = _LINUX_ADC_PATH
    start_localhost._WINDOWS_ADC_PATH = _WINDOWS_ADC_PATH
  ...
  def test_discover_adc_credentials_windows(
    ...
    start_localhost._WINDOWS_ADC_PATH = gen_test_util.test_file_path(mock_cred)
    self.assertEqual(
        bool(start_localhost._discover_adc_credentials()), expected
    )
  ...
  def test_discover_adc_credentials_linux(
    ...
    start_localhost._LINUX_ADC_PATH = gen_test_util.test_file_path(mock_cred)
    self.assertEqual(
        bool(start_localhost._discover_adc_credentials()), expected
    )
  ...
  def test_str_to_bool_true(self, val):
    self.assertTrue(start_localhost._str_to_bool(f'  {val.upper()}  '))

  @parameterized.parameters('n', 'no', 'f', 'false', 'off', '0')
  def test_str_to_bool_false(self, val):
    self.assertFalse(start_localhost._str_to_bool(f'  {val.upper()}  '))

  def test_str_to_bool_raises(self):
    with self.assertRaises(start_localhost._StartLocalHostError):
      start_localhost._str_to_bool('  foo  ')
  ...
  def test_mount(self, readonly, expected):
    self.assertEqual(start_localhost._mount('foo', 'bar', readonly), expected)

  def test_env(self):
    self.assertEqual(start_localhost._env('foo', 'bar'), ['-e', 'foo=bar'])

  @_ArgParserFlagsaver(google_application_credentials='abc')
  def test_are_gcp_credentials_does_not_define_file_raises(self):
    with self.assertRaises(start_localhost._StartLocalHostError):
      start_localhost._are_gcp_credentials_defined()
  ...
  def test_are_gcp_credentials_true(self):
    with _ArgParserFlagsaver(
      ...
      self.assertTrue(start_localhost._are_gcp_credentials_defined())

  @_ArgParserFlagsaver(google_application_credentials='')
  def test_are_gcp_credentials_defined_false(self):
    self.assertFalse(start_localhost._are_gcp_credentials_defined())
  ...
  def test_copy_file_to_image_ingestion_dir_success(self):
    ...
    with open(source, 'wt') as outfile:
      outfile.write('test')
    start_localhost._copy_file_to_image_ingestion_dir(source, dest.full_path)
  ...
  def test_failure_to_copy_file_fails_silently_dest_does_not_exist(self):
    ...
    with open(source, 'wt') as outfile:
      outfile.write('test')
    start_localhost._copy_file_to_image_ingestion_dir(source, dest)
    self.assertFalse(os.path.exists(os.path.join(dest, 'test.txt')))
  ...
  def test_is_dicom_store_gcp_store(self, dicom_store, expected):
    with _ArgParserFlagsaver(dicom_store=dicom_store):
      self.assertEqual(start_localhost._is_dicom_store_gcp_store(), expected)

  @_ArgParserFlagsaver(dicom_store=None)
  def test_no_configured_dicom_store_raises(self):
    with self.assertRaises(start_localhost._StartLocalHostError):
      start_localhost._test_dicom_store()
  ...
  def test_http_dicom_store_with_credentials_pass(self, unused_mock):
    self.assertIsNone(start_localhost._test_dicom_store())
  ...
  def test_get_dir_return_temp_dir(self):
    with contextlib.ExitStack() as stack:
      result = start_localhost._get_dir('foo', stack, '')
  ...
  def test_get_dir_with_existing_dir_succeeds(self):
    with contextlib.ExitStack() as stack:
      ...
      result = start_localhost._get_dir('foo', stack, tdir)
  ...
  def test_get_dir_with_bad_path_dir_raises(self):
    with contextlib.ExitStack() as stack:
      with self.assertRaises(start_localhost._StartLocalHostError):
        start_localhost._get_dir('foo', stack, 'BAD_DIR')
  ...
  def test_handle_log(self):
    ...
    with open(testfile, 'wt') as log:
      start_localhost._handle_log(mock_instance, log)
  ...
  def test_gen_docker_execution_args_temp_dir_mappings_and_fs_dicom_store(
    ...
    with contextlib.ExitStack() as stack:
      ingest_dir = start_localhost._get_dir('foo', stack, '')
      metadata_dir = start_localhost._get_dir('foo', stack, '')
      output_dir = start_localhost._get_dir('foo', stack, '')
      dicom_store = '/transform/dicom_store'

      args = start_localhost._gen_docker_execution_args(
          ingest_dir, metadata_dir, output_dir, dicom_store, False
      )
      mounts = _get_mounts(args)
      with open(
          gen_test_util.test_file_path('start_localhost/mount_temp_test.json'),
          'rt',
      ) as infile:
  ...
  def test_gen_docker_execution_args_fs_dir_mappings_and_cloud_dicom_store(
    ...
    with contextlib.ExitStack() as stack:
      ingest_dir = start_localhost._get_dir(
          'foo', stack, '/localhost/input_imaging'
      )
      metadata_dir = start_localhost._get_dir(
          'foo', stack, 'localhost/input_metadata'
      )
      output_dir = start_localhost._get_dir(
      ...
      mounts = _get_mounts(args)
      with open(
          gen_test_util.test_file_path('start_localhost/mount_fs_test.json'),
          'rt',
      ) as infile:
        self.assertEqual(mounts, json.load(infile))
  ...
  def test_gen_docker_execution_args(self, unused_mock) -> None:
    with contextlib.ExitStack() as stack:
      ...
      envs = dict(start_localhost._get_env_settings(args))
      del envs['LOCAL_UID']
      del envs['LOCAL_GID']
      with open(
          gen_test_util.test_file_path('start_localhost/env_test.json'), 'rt'
      ) as infile:
  ...
  def test_pyramid_level_config_args(self, unused_mock, config_path, expected):
    with contextlib.ExitStack() as stack:
      ...
      with _ArgParserFlagsaver(pyramid_generation_config_path=config_path):
        ...
        envs = start_localhost._get_env_settings(args)
        self.assertEqual(
  ...
  def test_missing_pyramid_config_raises(self):
    with contextlib.ExitStack() as stack:
      ...
      dicom_store = '/transform/dicom_store'
      with self.assertRaises(start_localhost._StartLocalHostError):
        start_localhost._gen_docker_execution_args(
            ingest_dir, metadata_dir, output_dir, dicom_store, False
        )
  ...
  def test_copy_file_list_into_input_dir_succeeds(self):
    ...
    with contextlib.ExitStack() as stack:
      start_localhost._copy_file_list_to_input_dir(
  ...
  def test_get_transform_env_log(self):
    ...
    self.assertEqual(
        start_localhost._get_transform_env_log(test_env), '\n'.join(expected)
    )

  def test_unexpected_formatted_env_raises(self):
    with self.assertRaises(start_localhost._StartLocalHostError):
      start_localhost._get_env_settings(['-e', 'abc'])
  ...
  def test_get_poll_image_ingestion_dir(self, is_temp_dir, poll, expected):
    ingest_dir = start_localhost._Directory('/foo', is_temp_dir)
    with _ArgParserFlagsaver(poll=poll):
      self.assertEqual(
          start_localhost._get_poll_image_ingestion_dir(ingest_dir), expected
      )
  ...
  def test_get_metadata_free_ingestion(
    ...
    metadata_dir = start_localhost._Directory('/foo', is_temp_dir)
    with _ArgParserFlagsaver(metadata_free_ingestion=metadata_free):
      self.assertEqual(
          start_localhost._get_metadata_free_ingestion(metadata_dir), expected
      )
  ...
  @mock.patch.object(
      start_localhost, '_are_gcp_credentials_defined', autospec=True
  )
  def test_get_big_query_metadata_table(
    ...
    with _ArgParserFlagsaver(big_query=bq_table):
      self.assertEqual(
          start_localhost._get_big_query_metadata_table(), bq_table
      )
  ...
  def test_get_big_query_metadata_table_raises(
    ...
    with _ArgParserFlagsaver(big_query=bq_table):
      with self.assertRaises(start_localhost._StartLocalHostError):
        start_localhost._get_big_query_metadata_table()
  ...
  def test_get_cloud_vision_barcode_segmentation(
    ...
    with mock.patch.object(
      ...
      with _ArgParserFlagsaver(cloud_vision_barcode_segmentation=segmentation):
        self.assertEqual(
            start_localhost._get_cloud_vision_barcode_segmentation_enabled(),
            expected,
        )
  ...
  def test_get_logging_ops_not_logging_to_cloud_ops(self):
    self.assertEqual(
        start_localhost._get_env_settings(start_localhost._get_logging_ops()),
  ...
  def test_get_logging_ops_no_google_credentials_raises(self, unused_mock):
    with self.assertRaises(start_localhost._StartLocalHostError):
      start_localhost._get_logging_ops()
        ...
        with mock.patch.object(
          ...
          with open(
              gen_test_util.test_file_path(
                  'start_localhost/mount_temp_test.json'
              ),
              'rt',
          ) as infile:
            self.assertEqual(mounts, json.load(infile))
          ...
          del envs['LOCAL_GID']
          with open(
              gen_test_util.test_file_path('start_localhost/env_test.json'),
              'rt',
          ) as infile:
            self.assertEqual(envs, json.load(infile))
  ...
  def test_main_write_docker_run_shellscript(self, *unused_mocks):
    ...
    with open(
        gen_test_util.test_file_path('start_localhost/expected_shellscript.sh'),
        'rt',
    ) as shellscript:
  ...
  def test_main_log_environment_variables(self, mock_logging, *unused_mocks):
    ...
    with open(
        gen_test_util.test_file_path(
            'start_localhost/log_environment_variables.txt'
        ),
        'rt',
    ) as infile:
    ...
    with io.BytesIO('one_line'.encode('utf-8')) as mock_stdout:
      ...
      with mock.patch.object(
        ...
        with mock.patch.dict(os.environ, envs):
          ...
          # This tests validates that enviromental variables set start_localhost
