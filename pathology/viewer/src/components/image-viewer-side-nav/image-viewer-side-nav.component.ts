...
private restoreSelectionHandlers() {
    try {
      ...
      // OpenLayers doesn't expose a single stable/public flush renderer caches API.
      // We use the public source.changed() and refresh() when present as a best-effort
      // nudge to re-render after restoring selection/overlay behavior.
      // hitTestCache.invalidate() is also best-effort (only if present) to avoid stale hit
      // detection in edge cases; it is guarded to keep this resilient across OL versions.
