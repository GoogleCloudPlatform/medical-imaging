...
class RedisClientTest(parameterized.TestCase):
  ...
  def test_acquire_redis_lock(self):
    ...
    self.assertTrue(
        cl.acquire_non_blocking_lock(_MOCK_LOCK_NAME, _MOCK_TOKEN_VALUE, 10)
    )
  ...
  def test_redis_lock_expire_owned_false(self):
    cl = redis_client.redis_client()
    cl.acquire_non_blocking_lock(_MOCK_LOCK_NAME, _MOCK_TOKEN_VALUE, 0.1)  # pytype: disable=wrong-arg-types
    time.sleep(0.3)
  ...
  def test_cannot_acquire_lock_taken(self):
    ...
    cl.acquire_non_blocking_lock(_MOCK_LOCK_NAME, _MOCK_TOKEN_VALUE, 10)
    self.assertFalse(
        cl.acquire_non_blocking_lock(_MOCK_LOCK_NAME, _MOCK_TOKEN2_VALUE, 10)
    )

  def test_extend_all_locks(self):
    cl = redis_client.redis_client()
    cl.acquire_non_blocking_lock(_MOCK_LOCK_NAME, _MOCK_TOKEN_VALUE, 0.5)  # pytype: disable=wrong-arg-types
  ...
  def test_redis_acquire_lock_empty_token_raise(self):
    cl = redis_client.redis_client()
    with self.assertRaises(ValueError):
      cl.acquire_non_blocking_lock(_MOCK_LOCK_NAME, '', 10)
